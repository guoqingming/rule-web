    ...
    return [
...

        {
            ...
            formatter: function (value, row, index) {
                var str = "";
                // if(!row.enable){
                //     str += '<input class="checkbox-enable" type="checkbox"   />\n';
                //
                // }else {
                //     str += '<input class="checkbox-enable" type="checkbox " checked />\n';
                // }
                if(!row.enable){
                    str += '<button class="btn btn-success" data-id="'+row.id+'" data-enable="'+row.enable+'" onclick="strategy.SwitchState(this)"  >启用</button>\n';

                }else {
                    str += '<button class="btn btn-warning" data-id="'+row.id+'" data-enable="'+row.enable+'" onclick="strategy.SwitchState(this)"  >禁用</button>\n';
                }
                 str += "<button class='btn btn-success' onclick='strategy.showStrategyConfigModal(" + row.id + ")'>策略配置</button>";


                if (!row.enable || row.enable == 0) {
                    str += "&nbsp;&nbsp;<button class='btn btn-warning' onclick='strategy.delete(" + row.id + ")'>删除</button>";
                }
                return str;
            }
        }];
...
strategy.SwitchState = function (obj) {
    var id = $(obj).attr("data-id");
    var enable = $(obj).attr("data-enable");
    var param = {};
    param.id = id;
    if(enable=="1"){
        param.enable = 0;
    ...
    $.post("strategy/update",param,function (data) {
        validateResult(data)
        strategy.table.bootstrapTable('refresh', strategy.tableParams);
    })
    
};
strategy.paramQueryParams = function (params) {
    if (params) {
        params.strategyId = $("#strategyId").val();
        return params;
    }
    params = {};
    params.strategyId = $("#strategyId").val();
...
strategy.showStrategyConfigModal = function (id) {
    ...
    if (!strategy.inputDataTable) {
        ...
        strategy.inputDataTable.bootstrapTable("refresh", strategy.paramQueryParams)
    }

    if (!strategy.outputDataTable) {
        strategy.outputDataTable = outputParamTable(id);
    } else {
        strategy.outputDataTable.bootstrapTable("refresh", strategy.paramQueryParams)
    }
}
...
function inputParamTable(strategyId) {
    ...
    return $('#inputTable').bootstrapTable({
        ...
        sortable: false, //是否启用排序
        queryParams: strategy.paramQueryParams,//传递参数（*）
        sidePagination: "server", //分页方式：client客户端分页，server服务端分页（*）
        pageNumber: 1, //初始化加载第一页，默认第一页
        pageSize: 10, //每页的记录行数（*）
        pageList: [10, 25, 50, 100], //可供选择的每页的行数（*）
        ...
        $(this).find(".wrapOutputTable tbody tr").each(function (i) {
            {
                ...
                if (right.type == 1) {
                    ...
                    right.value = $(this).find("td:eq(3) select").val();
                } else {
                    right.value = $(this).find("td:eq(3) input").val();
                }
...
strategy.showTestModal = function () {
    $("#outputResult #json-renderer").empty()
    ...
    $.post("strategy/rule/test", param, function (data) {
        if (data) {
            if (data.success) {
                ...
                try {
                    var input = eval('(' + JSON.stringify(data.data) + ')');
                }
                catch (error) {
...
strategy.getDefaultRulePanelHtml = function (index) {
    if (!index) {
        index = 1;
    }
    var defaultRuleSettingsHtml = '<div class="panel panel-primary rule-panel">\n' +
        '                                            <div class="panel-heading">\n' +
        '                                                <h3 class="panel-title">规则&nbsp;<span class="badge">' + index + '</span>&nbsp;&nbsp;<a href="#" onclick="strategy.deleteRulePanel(this)" class="addOutputTr glyphicon glyphicon-trash"></a></h3>\n' +
        '                                            </div>\n' +
...
strategy.renderRuleSettings = function (ruleParams) {
    ...
    $.each(ruleParams, function (index, item) {
        var defaultRuleSettingsHtml = '<div class="panel panel-primary rule-panel">\n' +
            '                                            <div class="panel-heading">\n' +
            '                                                <h3 class="panel-title">规则 &nbsp;<span class="badge">' + (index + 1) + '</span> &nbsp;&nbsp;<a href="#" onclick="strategy.deleteRulePanel(this)" class=" glyphicon glyphicon-trash"></a></h3>\n' +
            '                                            </div>\n' +
            '                                            <div class="panel-body">\n' +
            // '                                               <div class="col-md-12">' +
            '                                                   <div class="form-group ">' +
            ...
            '                                                                </thead>\n' +
            '                                                                <tbody>\n';
        $.each(item.conditions, function (idx, it) {
            defaultRuleSettingsHtml += strategy.getConditionTr(idx);
        })
        ...
        defaultRuleSettingsHtml += '                                                                </tbody>\n' +
            ...
            '                                                                </thead>\n' +
            '                                                                <tbody>\n'
        $.each(item.outputSettings, function (idx, it) {
            defaultRuleSettingsHtml += strategy.outputResultTr;
        })
    ...
    $.each(ruleParams, function (index, item) {
        ...
        $.each(item.conditions, function (i, item) {
            $("#ruleSettingParentDiv .filterTable:eq(" + index + ") tbody tr:eq(" + i + ") td:eq(0)").text(item.group).attr("group-flag", item.groupFlag);
            ...
            $("#ruleSettingParentDiv .filterTable:eq(" + index + ") tbody tr:eq(" + i + ") td:eq(5) input[value='" + item.logicOperator.value + "']").prop("checked", true)
        })
        $.each(item.outputSettings, function (i, item) {
            ...
            if (2 == item.right.type) {
                $("#ruleSettingParentDiv .wrapOutputTable:eq(" + index + ") tbody tr:eq(" + i + ") td:eq(3) input").val("").hide()
                $("#ruleSettingParentDiv .wrapOutputTable:eq(" + index + ") tbody tr:eq(" + i + ") td:eq(3) select").val(item.right.value).show();
            } else {
                $("#ruleSettingParentDiv .wrapOutputTable:eq(" + index + ") tbody tr:eq(" + i + ") td:eq(3) select").val("").hide()
...
strategy.addRulePanel = function () {
    var num = $("#ruleSettingParentDiv .rule-panel").length;
    $("#ruleSettingParentDiv").append(strategy.getDefaultRulePanelHtml());
    $("#ruleSettingParentDiv .rule-panel:last span.badge").text((num + 1))
}
...
$("body").on("change", ".param-type", function () {
    var paramType = $(this).find("option:selected").val()
    if (paramType == "2") {
...
strategy.getConditionTr = function (index) {
    return "<tr>" + $(strategy.defaultConditionTr).find(":radio").attr("name", "connector" + index).parents("tr").html() + "</tr>";
